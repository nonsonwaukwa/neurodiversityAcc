...
def trigger_followup_reminders():
    ...
    # Make the request
    try:
        logger.info(f"Triggering morning follow-up reminders webhook at {webhook_url}")
        response = requests.post(webhook_url, headers=headers, json=payload, timeout=30)
        
        if response.status_code == 200:
            logger.info(f"Morning follow-up reminders webhook triggered successfully: {response.text}")
            return True
        else:
            logger.error(f"Failed to trigger morning follow-up reminders webhook. Status: {response.status_code}, Response: {response.text}")
            return False
            
    except requests.exceptions.RequestException as e:
        logger.error(f"Error triggering morning follow-up reminders webhook: {str(e)}")
        return False
