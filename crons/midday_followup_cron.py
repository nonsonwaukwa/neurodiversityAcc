#!/usr/bin/env python3
"""
Midday Follow-up Reminder Cron Script (2:30 PM)
...
def check_eligible_users():
    ...
    try:
        ...
        for user in active_users_list:
            ...
            # Check if within reminder window (4 hours after check-in)
            if timedelta(hours=3.5) <= time_since_checkin <= timedelta(hours=4.5):
                logger.info(f"User {user_id} is eligible for a midday follow-up reminder")
                ...
                logger.info(f"User {user_id} is not in the midday follow-up window")
        
        logger.info(f"Found {len(eligible_users)} users eligible for midday follow-up reminders")
...
if __name__ == "__main__":
    ...
    logger.info("="*80)
    logger.info("Midday Follow-up Reminder Cron Job Starting")
    logger.info("="*80)
    ...
    logger.info("="*80)
    if success:
        logger.info("Midday Follow-up Reminder Cron Job Completed Successfully")
    else:
        logger.error("Midday Follow-up Reminder Cron Job Failed")
    logger.info("="*80)
